...
| first `model_wait` | ~1.8 s (incl. cold cache **write** 6,384 tok) | **shorter** (cache **read**, not write) | prompt-cache warmth: shared Boondi prefix kept hot (§8) |
...
| `GANTRY_IPC_INPUT_DIR` | `continuation-input.ts:32` | spawn env (polled after boot) | **easy** — set at bind |
...
`GANTRY_IPC_INPUT_DIR` (path), recomputed `GANTRY_MEMORY_IPC_AUTH_TOKEN` (chatJid-scoped, server-validated), the MCP caller-identity (re-signed, fresh `ts`), the gantry-MCP child's customer identity (§7.2), a fresh run handle, the egress principal, and the first message + memory block + guardrail preface.
...
- **What it buys:** the first model call reads the warm shared prefix (cache **read**, ~0.1×) instead of the cold **write** (1.25×, the 6,384-tok `cache w` in the trace) → shorter first `model_wait`.
...
2. The reply's trace has **no `startup` section** and shows the warm split (`queue` + `model_wait` via `dispatchedAt`) — the exact assertion in `reply-trace.test.ts:394` (`assembleTimeline`: `startup` only when `startup.readyAt > startup.startedAt`, `reply-trace.ts:286`).
...
- Bound-state inventory: `agent-spawn.ts`, `agent-capabilities.ts`, `runner/query-loop.ts`, `runner/system-prompt.ts`, `runner/message-stream.ts`, `runtime/continuation-input.ts`, `runtime/ipc-auth.ts`, `application/mcp/mcp-caller-identity.ts`, `application/capability-secrets/mcp-secret-projection.ts`, `runtime/memory-boundary.ts`, `prompt-cache.ts`.
...
- **F3 — the runner entry is once-only stdin, and stdin is closed (CRITICAL, runner restructure).** Runner reads input once: `readStdin()`→`JSON.parse`→`runQuery` (`index.ts:65-66,94,198`); core closes stdin immediately (`agent-spawn-process.ts:201-202`). So bind **cannot** use stdin. `index.ts` must become a **two-phase entry**: (1) read a *generic* `AgentRunnerInput`, `startup()`-boot, signal ready; (2) receive the **bind** (identity + first message) over a non-stdin channel (Pillar 1's socket; the standalone shim uses the continuation-input dir) and then run. (Amends §6/§16; the spike must exercise exactly this.)
