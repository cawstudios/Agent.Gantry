...
| `settings.yaml` | Unchanged by design. No new setting. | Changed only if a reviewed projection or low-latency mode needs a settings-owned field. |
...
| Control API | Unchanged by design. | Changed if task status, rejoin cursors, receipts, or structured results are exposed. |
| SDK/contracts | Unchanged unless public DTO typing is added. | Changed for public lifecycle/status/structured-output DTOs. |
| CLI | Unchanged by design. | Changed only for status/diagnostic display. |
| Gantry MCP tools/admin skill | Unchanged by design. | Changed only for reviewed diagnostics or agent-facing Gantry delegation tools. |
