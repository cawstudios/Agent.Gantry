...
| Memory and retrieval | `apps/core/src/runtime/memory-context.ts`, `apps/core/src/memory/memory-service.ts`, `apps/core/src/memory/memory-service-search.ts`, `apps/core/src/memory/persistence/`, `apps/core/src/memory/memory-retrieval.ts` | Stores durable memory, performs pgvector semantic lookup and Postgres full-text lookup, and injects bounded context into prompts. |
...
Postgres is mandatory runtime storage. The schema contains first-party MyClaw tables for groups, chats, messages, sessions, jobs, runs, control events, webhooks, deliveries, memory, and audit records.

Retrieval uses two Postgres-native paths:

- pgvector for semantic memory lookup and dedupe
...
| Agent starts but does not answer | `apps/core/src/runtime/group-processing.ts`, `apps/core/src/runtime/agent-spawn.ts`, `apps/core/src/runner/claude/query-loop.ts` | Prompt construction, child process env, Claude session id, runner stdout markers, final-output handling. |
...
- Storage and search: `apps/core/src/infrastructure/postgres/schema/schema.ts`, `apps/core/src/infrastructure/postgres/runtime-store.ts`, `apps/core/src/memory/persistence/`, `apps/core/src/memory/memory-retrieval.ts`
