...
- `MIGRATION_DATABASE_URL` is optional production split-role migration authority, not required for normal installs; when present, Docker/bootstrap schema resolution must match migration schema precedence before runtime `GANTRY_DATABASE_URL`.
