...
export function createRuntimeJobSchedulePlanner(
...
function resolveStandaloneTimezone(): string {
  const candidates = [
    process.env.TZ,
    Intl.DateTimeFormat().resolvedOptions().timeZone,
  ];
  return candidates.find((value) => value && isValidTimezone(value)) ?? 'UTC';
}

export const runtimeJobSchedulePlanner = createRuntimeJobSchedulePlanner(
  resolveStandaloneTimezone(),
);
