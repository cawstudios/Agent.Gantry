...
export abstract class SlackChannelPermissionInteractions extends SlackChannelState {
  ...
  protected registerSlackPermissionHandlers(): void {
    if (!this.app) return;
    const handlePermissionDecision = async (args: any) => {
      ...
      if (!permissionDecisionOptions(pending.request).includes(mode)) return;
      const callbackChannelId =
        body.channel?.id ||
        body.container?.channel_id ||
        body.message?.channel ||
