...
function defaultPostgresNextAction(): string {
  return [
    'Use a Postgres endpoint with pgvector + pg_trgm/pg_search + pg-boss initialized.',
    'Local Docker Postgres is supported for development; set sslmode=require or stronger for remote URLs.',
  ].join(' ');
}
...
export async function inspectRuntimeStorageReadiness(
  ...
  try {
    ...
    return {
      ...
      details: [message],
      nextAction: `Verify ${postgresUrlEnv} and database network access.`,
    };
  } finally {
    await service.close();
  }
