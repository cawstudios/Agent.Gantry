...
export const memoryItemsPostgres = pgTable(
  ...
  (table) => ({
    ...
    embeddingIdx: index('idx_memory_items_embedding').using(
      'hnsw',
...
export const memoryProceduresPostgres = pgTable(
  'memory_procedures',
  {
    id: text('id').primaryKey(),
    scope: text('scope').notNull(),
  ...
  (table) => ({
    scopeGroupIdx: index('idx_memory_procedures_scope_group').on(
      table.scope,
      table.groupFolder,
      table.topicId,
      table.updatedAt,
    ),
    searchIdx: index('idx_memory_procedures_search').using(
      'gin',
...
export const memoryChunksPostgres = pgTable(
  'memory_chunks',
  {
    id: text('id').primaryKey(),
    ...
    importanceWeight: doublePrecision('importance_weight').notNull().default(1),
    embeddingJson: text('embedding_json'),
    embedding: vector('embedding', { dimensions: 3072 }),
    createdAt: timestamp('created_at', {
      withTimezone: true,
  ...
  (table) => ({
    scopeGroupIdx: index('idx_memory_chunks_scope_group').on(
      table.scope,
      table.groupFolder,
      table.topicId,
      table.updatedAt,
    ),
    sourceIdx: index('idx_memory_chunks_source').on(
      table.sourceType,
      table.sourceId,
    ),
    searchIdx: index('idx_memory_chunks_search').using(
      'gin',
      sql`to_tsvector('english', ${table.text})`,
    ),
    embeddingIdx: index('idx_memory_chunks_embedding').using(
      'hnsw',
      table.embedding.op('vector_cosine_ops'),
    ),
  }),
...
export const memoryEventsPostgres = pgTable(
  'memory_events',
  {
    id: integer('id').generatedAlwaysAsIdentity().primaryKey(),
    eventType: text('event_type').notNull(),
  ...
  (table) => ({
    eventEntityIdx: index('idx_memory_events_event_entity').on(
      table.eventType,
      table.entityId,
      table.id,
    ),
...
export const memoryUsageEventsPostgres = pgTable('memory_usage_events', {
  id: integer('id').generatedAlwaysAsIdentity().primaryKey(),
