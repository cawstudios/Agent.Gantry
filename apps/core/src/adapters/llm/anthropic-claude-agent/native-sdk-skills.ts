...
export const GANTRY_CLAUDE_SDK_PROGRESSIVE_SKILLS_ENV =
  'GANTRY_CLAUDE_SDK_PROGRESSIVE_SKILLS_JSON';
...
export function readClaudeSdkProgressiveSkillNamesFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): string[] {
  const raw = env[GANTRY_CLAUDE_SDK_PROGRESSIVE_SKILLS_ENV];
  if (!raw) return [];
  const parsed = JSON.parse(raw) as unknown;
  if (
    !Array.isArray(parsed) ||
    ...
    throw new Error(
      `${GANTRY_CLAUDE_SDK_PROGRESSIVE_SKILLS_ENV} must be a JSON string array.`,
    );
  }
  return [...new Set(parsed.filter((item) => item.trim().length > 0))].sort();
}
...
export function claudeSdkToolsForEnabledSkills(
  ...
  progressiveSkills: readonly string[] = enabledSkills,
): string[] {
  if (
    enabledSkills.length === 0 ||
    progressiveSkills.length === 0 ||
    availableTools.includes('Skill')
  ) {
