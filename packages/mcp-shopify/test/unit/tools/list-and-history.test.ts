  ...
  it('returns orders sorted by createdAt descending', async () => {
    const mock = buildMockFetch({
      graphqlResponses: [
        ...
        graphqlOk(
          ordersEdges([
            { name: 'BSS-3001', customer: BUSY_CUSTOMER, createdAt: '2026-05-16T08:00:00Z' },
            { name: 'BSS-3002', customer: BUSY_CUSTOMER, createdAt: '2026-05-17T08:00:00Z' },
            { name: 'BSS-3003', customer: BUSY_CUSTOMER, createdAt: '2026-05-15T08:00:00Z' },
          ]),
        ),
      ],
    });
  ...
  it('returns orders in date range', async () => {
    const mock = buildMockFetch({
      graphqlResponses: [
        ...
        graphqlOk(
          ordersEdges([
            { name: 'BSS-1100', customer: BUSY_CUSTOMER, createdAt: '2025-10-15T08:00:00Z' },
          ]),
        ),
      ],
    });
  ...
  it('returns SCOPE_MISSING when GraphQL reports access denied for old orders', async () => {
    const mock = buildMockFetch({
      graphqlResponses: [
        graphqlOk(customersEdges([BUSY_CUSTOMER])),
        graphqlErrors([
          { message: 'Access denied: read_all_orders scope required. Request access.' },
        ]),
      ],
    });
    const harness = buildToolHarness(mock.fetch);
